...
let raindropContext = null;
/**
 * Initialize Raindrop integrations
 * Called at application startup
 */
export async function initializeRaindrop(context) {
    try {
        if (context) {
 ...
 */
export async function getPOIsFromSmartBuckets(city, theme, budget, count = 10) {
    ...
    try {
        ...
        return results.map((doc) => doc.content.poi);
    }
    catch (error) {
 ...
 */
export async function saveSessionToSmartMemory(sessionId, session) {
    ...
    try {
        ...
        await raindropContext.smartmemory_session.put(`session:${sessionId}`, context, {
            metadata: {
                type: 'chat_session',
 ...
 */
export async function loadSessionFromSmartMemory(sessionId) {
    if (!raindropContext?.smartmemory_session) {
        return null;
    }
    try {
 ...
 */
export async function saveTripToSmartSQL(trip) {
 ...
 */
export async function getTripsForAnalysis(theme, city, limit = 100) {
    if (!raindropContext?.smartsql_smart_trips) {
        return [];
    }
    try {
 ...
 */
export async function saveAIInsights(sessionId, insights) {
    if (!raindropContext?.smartmemory_trip) {
        return true;
    }
    try {
        await raindropContext.smartmemory_trip.put(`insights:${sessionId}`, {
            sessionId,
            ...insights,
            timestamp: new Date().toISOString()
        }, {
 ...
 */
export async function getSessionFromCache(sessionId) {
 ...
 */
export async function cacheSession(sessionId, session) {
    if (!raindropContext?.kv_session_cache) {
        return true;
    }
    try {
        // Cache for 24 hours
        await raindropContext.kv_session_cache.put(`session:${sessionId}`, JSON.stringify(session), { expirationTtl: 86400 });
        return true;
    }
    catch (error) {
 ...
 */
export async function getSimilarTrips(checklist, limit = 5) {
    if (!raindropContext?.smartsql_smart_trips) {
        return [];
    }
    try {
 ...
 */
export async function addPOIsToSmartBucket(pois) {
    if (!raindropContext?.smartbucket_pois) {
        return true;
    }
    try {
 ...
 */
function getMockPOIs(city, theme) {
    const mockData = {
        pune: {
            adventure: [
                {
 ...
 */
function extractPreferences(checklist) {
    return {
 ...
 */
export function isRaindropReady() {
