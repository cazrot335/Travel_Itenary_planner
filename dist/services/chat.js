 ...
 */
import { createEmptyChecklist, calculateCompleteness } from './../types/checklist.js';
import { parseMessage } from '../nlp-parser.js';
import { saveSessionToSmartMemory, loadSessionFromSmartMemory, cacheSession, getSessionFromCache, isRaindropReady } from '../raindrop-integration.js';
import { getAIResponse, generateSmartSuggestions, predictNextFields, refineChecklistWithAI } from '../ai-service.js';
/**
 ...
 */
export async function initializeSession(sessionId) {
 ...
 */
async function generateItinerary(sessionId, checklist) {
    try {
 ...
 */
function generateDayBlocks(dayIndex, checklist) {
    const blocks = [];
 ...
 */
function getThemeForDay(dayIndex, totalDays, checklist) {
    const theme = checklist.tripTheme || 'Mixed';
    if (dayIndex === 0)
        return 'Arrival & Settling In';
...
async function persistSession(session) {
    ...
    if (isRaindropReady()) {
        promises.push(saveSessionToSmartMemory(session.sessionId, session).catch(e => console.warn('SmartMemory error:', e)));
    }
