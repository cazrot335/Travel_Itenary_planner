 ...
 */
function parseStartDate(message) {
    const lower = message.toLowerCase();
 ...
 */
function parseTravelDays(message) {
 ...
 */
function parseSpecialNeeds(message, pattern) {
 ...
 */
function parsePlacesList(message, pattern) {
    const regex = new RegExp(`(?:${pattern})[^.!?]*[.!?]?`, 'i');
    const match = message.match(regex);
    if (match) {
 ...
 */
function getNextDayOfWeek(message) {
    const dayMap = {
