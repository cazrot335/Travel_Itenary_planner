...
app.post('/api/chat', async (c) => {
    try {
        ...
        if (!sessionId || typeof sessionId !== 'string') {
            return c.json({ error: 'Missing or invalid sessionId' }, { status: 400 });
        }
        if (!message || typeof message !== 'string' || message.trim().length === 0) {
            return c.json({ error: 'Missing or invalid message' }, { status: 400 });
        }
        // Process chat
...
app.post('/api/feedback', async (c) => {
    try {
        ...
        if (!sessionId || !tripId) {
            return c.json({ error: 'Missing sessionId or tripId' }, { status: 400 });
        }
